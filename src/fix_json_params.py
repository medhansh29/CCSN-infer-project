...
def fix_json_parameters(base_dir='.'):
    ...
    for date_dir in date_dirs:
        all_json_files.extend(list(date_dir.glob("*_nn.json")))
        
    fixed_count = 0
    missing_txt_count = 0
    
    for json_file in tqdm(all_json_files, desc="Fixing JSONs"):
        parts = json_file.stem.split('_')
        if len(parts) >= 3:
            ztf_id = parts[0]
            
            # Check for corresponding samples.txt
            samples_txt = json_file.parent / f"{ztf_id}_samples.txt"
            ...
            try:
                ...
                print(f"\n  [!] Error processing {samples_txt}: {e}")
